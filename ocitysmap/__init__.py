...
class RenderingConfiguration:
    ...
    def __init__(self):
        ...
        self.addpolys        = None # like bbox, but WktString (add to be rendered)
        self.subpolys        = None # like above (but NOT to be rendered)
...
""")
    ...
    def render(self, config, renderer_name, output_formats, file_prefix):
        ...
        assert config.osmids or config.addpolys, \
                'At least an OSM ID or a add-polygons must be provided!'
        ...
        if config.addpolys is not None:
            ...
            if add_wkt.is_empty:
                raise ValueError("add_wkt is empty!")
        ...
        config.polygon_wkt = result_wkt.wkt
        config.bounding_box = coords.BoundingBox.parse_wkt(result_wkt.wkt)
    ...
    def __del__(self):
        if True:
            with open('/home/maposmatic/ocitysmap/support/debug.html') as f:
                s = Template(f.read())
                fout = open('/home/maposmatic/maposmatic/rendering/results/debug.html', 'w')
                fout.write(s.substitute(content=self.js_debug_string))
                fout.close()
